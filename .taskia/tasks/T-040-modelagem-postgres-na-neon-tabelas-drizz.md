...
atualizado_em: 2026-09-07T17:16:53.496Z
versao: 4
estimativa: M
dependencias: []
...
## Contexto
Decisão (humano): Neon free tier. Acesso via driver serverless (`@neondatabase/serverless`, HTTP — sem pool TCP, funciona em qualquer hospedagem) + `drizzle-orm` (tipado, leve, SQL visível). `DATABASE_URL` só em `.env` (nunca commit). Catálogo local (pratos/clientes no aparelho) continua local por ora — servidor passa a ser dono de: contas, sessões, publicados e escolhas.
